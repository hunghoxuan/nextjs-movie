...
import { IWebService, Taxonomy, PageResult, IDataService, QueryItem} from "@/lib/types";
import { NextRequest, NextResponse } from "next/server";
import { getMedia, getSearch, getGenreList, getGenre, apiImgUrl, apiUrl, getQuery } from "./tmdb.db";
...
export const tmdbService : IDataService<Media, MediaType | string> = {
  ...
  query: async function (query: QueryItem, page?: number | string): Promise<PageResult<Media>> {
    return await getQuery(query, page);
  },
  search: async function (query: QueryItem, page?: number | string): Promise<PageResult<Media>> {
...
export const tmdbWebService : IWebService = {
  getResponse: function (req: NextRequest): NextResponse | null {
    ...
    if (mediaType === "") { // home page
      const url = req.nextUrl.clone();
      url.pathname = `/movies`;
      return NextResponse.rewrite(url);
    }
    if (MediaTypes.includes(mediaType)) { // rewrite url to use site name (movies)
        const url = req.nextUrl.clone();
